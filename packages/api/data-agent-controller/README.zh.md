...
连通性操作发送一条要求回复 `OK` 的合成指令。业务消息仍由领域 Harness 集成负责。

#### Token effect

显式连通性测试通过选定模型增加一次简短请求。业务会话消息由领域集成渲染。
...
testModel Remote 通过既有代理循环与供应商注册表发送合成连通性消息，不提供工具或业务数据。model-connectivity 消息来源不会触发面向用户消息的自动标题生成。每次测试保留正常会话审计，仅返回成功状态、安全错误码、耗时和会话 ID。每个控制器同时只运行一个测试；浏览器取消、控制器卸载或配置的超时会停止测试。该配置操作需要 Harness 连接授权，但不需要 Data Agent 业务凭证；第一阶段不隔离管理员角色。
