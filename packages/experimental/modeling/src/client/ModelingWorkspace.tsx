    ...
    <section className={css.workspace} aria-label={t('view.title')}>
      ...
      <div className={css.columns}>
        <div className={css.mainColumn}>
          ...
          <article className={css.card}>
            <CardTitle icon={<IconPlayOutline16 />} title={t('plan.title')} trailing={<Tag tone={state.plan?.state === 'proposed' ? 'warning' : 'success'}>{state.plan === null ? t('unknown') : `${state.plan.state} · r${state.plan.revision}`}</Tag>} />
      ...
      <Modal open={editing} onClose={() => { setEditing(false) }} title={t('plan.dialog.title')} closeLabel={t('close')} description={t('plan.dialog.description')} footer={<><Button variant="ghost" onClick={() => { setEditing(false) }}>{t('action.cancel')}</Button><Button variant="primary" icon={<IconCheckOutline16 />} onClick={() => void save()}>{t('action.save')}</Button></>}>
        {draft !== null && <div className={css.planForm}>
          ...
          <FormField label="C"><input type="number" min={Number(cBounds.min)} max={Number(cBounds.max)} step="0.1" value={Number(object(object(first(draft.models)).params).C)} onChange={(event) => { mutateDraft((next) => { object(object(first(next.models)).params).C = Number(event.target.value) }) }} /></FormField>
