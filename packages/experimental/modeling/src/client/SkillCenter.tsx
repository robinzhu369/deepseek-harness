...
export function SkillCenter({ model, state, t }: SkillCenterProps) {
  ...
  return <section className={css.editorPanel}>
    <div className={css.editorHead}><div><h3>{t(`skill.${detail.name}` as ModelingKey)}</h3><code>{detail.name}</code><span> · {detail.published_version}</span></div><div className={css.headerBadges}><Tag tone={detail.draft_hash === null ? 'success' : 'warning'}>{detail.draft_hash === null ? t('skills.published') : t('skills.draft')}</Tag><Tag tone="info">{t('skills.metadata')}</Tag></div></div>
    <section className={css.detailCard}><h4>{t('skills.guide')}</h4><p>{t('skills.guideNotice')}</p>{t(`skills.guide.${detail.name}` as ModelingKey).split('\n').map(line => <p key={line}>{line}</p>)}<p>{t('skills.guardrails')}</p></section>
    <details className={css.instructions}><summary><IconDataOutline16 />{t('skills.source')}</summary><textarea value={draft} aria-label={t('skills.markdown')} onChange={(event) => { setDraft(event.target.value) }} />
