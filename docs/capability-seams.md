...
| `ctx.modeling` | `core` | [`experimental-modeling`](../packages/experimental/modeling) | - | [`experimental-modeling`](../packages/experimental/modeling) | - | Owns Session-bound requests to the private Modeling API; bounded model tools can inspect and propose, while application-only Remote methods retain approval authority. |
