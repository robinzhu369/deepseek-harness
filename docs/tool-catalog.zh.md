...
来源：[`packages/experimental/modeling/src/tools.ts`](../packages/experimental/modeling/src/tools.ts)

### `modeling_get_run_result`

读取当前 Session 所有的一个成功运行的指标、manifest 摘要和已完成产物元数据。
...
### `modeling_get_run_status`

读取当前 Session 所有的一个运行的当前状态和有界节点事件。
...
校验并持久化新计划或乐观并发 revision。此工具绝不批准或启动运行。
...
四个工具从调用 Agent 推导 Session 身份，且不暴露审批能力。应用仅在用户明确确认后通过私有 Remote 适配器调用 approveAndRun。
