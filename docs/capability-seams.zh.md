...
| `ctx.dataAgentController` | `core` | [`api-data-agent-controller`](../packages/api/data-agent-controller) | - | [`client-ui-data-agent`](../packages/client/ui-data-agent) | - | 将有大小限制且已认证的领域请求转发到配置的本机监听器；执行和项目权限仍由领域仓库负责。 |
