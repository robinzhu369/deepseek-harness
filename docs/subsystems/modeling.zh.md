...
实验性建模子系统把专用 Agent 和 Web Client 连接到确定性建模 API。面向模型的工具可以检查数据、提出经过校验的计划并读取 run 结果；只有面向应用的 Remote 方法可以批准计划或请求重跑。

## 所有权

| 所有者 | 职责 |
|---|---|
| [建模包](../../packages/experimental/modeling/README.zh.md) | Host 适配器、有界工具、审批 Remote、Client 状态和工作台 UI |
| [建模 API](../../services/modeling-api/README.zh.md) | 数据集画像、计划校验、执行、持久化和 artifact 访问 |

## 运行时模型

`ctx.modeling` 在每个私有 API 请求中发送实时 Session 标识。四个面向模型的工具从调用方 Agent 派生该标识，且不暴露审批能力。Client 使用 Session 范围状态管理计划 revision、轮询、结果和 artifact 下载。

运行时 Skills 从显式根目录加载，并在计划提议记录其快照前计算哈希。建模 preset 屏蔽继承的通用工具，因此 Agent 无法通过此子系统获得 shell、文件系统写入、任意 HTTP、SQL 或 Python 执行能力。
...
Types: [Agent](core.zh.md) · [FileAttachmentRef](attachment.zh.md)
