...
Profiles include full-data blank and marker counts, nonblank cardinality, binary-value frequencies, finite numeric summaries, exact duplicate rows, and conditional insurance date comparisons. Marker strings are candidates rather than automatic missing values; conversion counts do not establish semantic types. Date parsing accepts YYYY/MM/DD and YYYY-MM-DD. Preview cells and frequency values retain CSV strings; inferred dtypes are advisory. Profiling scans columns separately and exact uniqueness can require memory proportional to the data; large-data capacity is not established. Existing cached profiles are not recomputed automatically.
...
The [data-analysis Skill](../../.dsh/skills/data-analysis/SKILL.md) reports evidence-backed quality findings and distinguishes quality-only review, missing confirmation, blocked requests, and readiness to draft a plan. Its [assessment schema](../../.dsh/skills/data-analysis/output.schema.json) is checked against authored cases by `tests/test_data_analysis_skill.py`; these checks do not establish live model quality or install an API decision validator. The [evaluation cases](tests/fixtures/data-analysis-evals.json) include business context, bounded profiles, expected assessments, and behavioral checks for a future live comparison.
